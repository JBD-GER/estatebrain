...
export function ValuationDialog({propertyId, name, defaults, disabled}: {propertyId:string; name:string; defaults:Partial<ValuationInput>; disabled?:boolean}) {
  ...
  const labels: Record<string,string> = {typ:"Haus oder Wohnung",street:"Straße und Hausnummer",postcode:"Postleitzahl",city:"Ort",square_meters:"Wohnfläche"};
  ...
  return <Dialog open={open} onOpenChange={v=>{if(!pending)setOpen(v);}}>
    <DialogTrigger asChild><Button disabled={disabled}>Immobilie bewerten</Button></DialogTrigger>
    <DialogContent className="max-h-[90dvh] overflow-y-auto sm:max-w-2xl" showCloseButton={!pending}>
      <DialogHeader><DialogTitle>Bewertung für {name}</DialogTitle><DialogDescription>Eine Somantic-Bewertung pro Kalenderjahr. Prüfe die Angaben vor dem Abruf. Der geschätzte Kaufpreis wird als aktueller Marktwert übernommen.</DialogDescription></DialogHeader>
      ...
      <form className="space-y-4" onSubmit={event=>{
        ...
        <Button type="submit" disabled={pending} className="w-full">{pending ? "Bewertung wird abgerufen …" : "Jahresbewertung jetzt abrufen"}</Button>
      </form>
    </DialogContent>
  </Dialog>;
}
