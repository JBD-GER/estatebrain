...
import { InvestmentCashflowPanel } from "@/components/tax/investment-cashflow-panel";
...
export function InvestmentWorkspace({ initialScenarios = [], storageError, portfolioProperties = [], portfolioError, skippedProperties = 0 }: {
  ...
  const [tab, setTab] = useState<"overview" | "cashflow" | "schedule" | "basis">("overview");
  ...
  return <div className="mx-auto max-w-[1440px] space-y-7 pb-10" data-testid="investment-workspace" data-hydrated={hydrated}>
    <header className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
      <div>
        ...
        <h1 className="text-3xl font-semibold tracking-[-0.045em] sm:text-4xl">Steuerszenarien & Abschreibung.</h1>
        <p className="mt-3 max-w-2xl text-sm leading-relaxed text-muted-foreground">Bestand, Neubau oder Denkmal. Plane Abschreibung, prognostizierte Miete und Finanzierung – mit jährlichem Cashflow nach Steuern.</p>
      </div>
      ...
      <div id="comparison-results" className="min-w-0 scroll-mt-5 space-y-5">
        ...
        {calculation && result ? <>
          ...
          <section className="rounded-2xl border bg-card p-5">
            ...
            {result.switchedToLinearYear ? <p className="mt-3 text-xs text-primary">Ab {result.switchedToLinearYear} wechselt die Berechnung zur linearen Abschreibung des Restwerts über die verbleibende Nutzungsdauer.</p> : null}
          </section>

          <section className="overflow-hidden rounded-2xl border bg-card">
            <div className="flex gap-1 overflow-x-auto border-b px-4 pt-2" role="tablist" aria-label="Auswertung">
              {([{ id: "overview", label: "Steuerwirkung" }, { id: "cashflow", label: "Miete & Finanzierung" }, { id: "schedule", label: "Jahr für Jahr" }, { id: "basis", label: "Rechenweg" }] as const).map((item) => <button key={item.id} type="button" role="tab" aria-selected={tab === item.id} tabIndex={tab === item.id ? 0 : -1} onKeyDown={(event) => { const tabs = ["overview", "cashflow", "schedule", "basis"] as const; const index = tabs.indexOf(tab); const next = event.key === "ArrowRight" ? (index + 1) % tabs.length : event.key === "ArrowLeft" ? (index + tabs.length - 1) % tabs.length : event.key === "Home" ? 0 : event.key === "End" ? tabs.length - 1 : -1; if (next >= 0) { event.preventDefault(); setTab(tabs[next]); document.getElementById(`tab-${tabs[next]}`)?.focus(); } }} id={`tab-${item.id}`} aria-controls={`panel-${item.id}`} onClick={() => setTab(item.id)} className={cn("whitespace-nowrap border-b-2 px-4 py-3 text-xs font-medium transition", tab === item.id ? "border-primary text-primary" : "border-transparent text-muted-foreground hover:text-foreground")}>{item.label}</button>)}
            </div>
            <div role="tabpanel" id={`panel-${tab}`} aria-labelledby={`tab-${tab}`} className="p-5 sm:p-6">
              ...
              {tab === "cashflow" ? <InvestmentCashflowPanel input={draft.input} scenario={result} /> : null}
              {tab === "schedule" ? <>
          ...
          <details className="group rounded-2xl border bg-card p-5"><summary className="flex cursor-pointer list-none items-center gap-2 text-xs font-semibold [&::-webkit-details-marker]:hidden"><Landmark className="size-4 text-primary" />Regeln, Voraussetzungen & Quellen<ChevronDown className="ml-auto size-4 transition group-open:rotate-180" /></summary><div className="mt-5 space-y-4 text-xs leading-relaxed text-muted-foreground">
            ...
            <p className="text-[10px]">Gesetzesprüfung: 23.09.2026. Individuelle Sonderfälle und spätere Gesetzesänderungen müssen gesondert geprüft werden.</p>
          </div></details>
        </> : null}
        <div className="flex items-start gap-2.5 px-1 text-[11px] leading-relaxed text-muted-foreground"><ShieldCheck className="mt-0.5 size-4 shrink-0" /><p>Unverbindliche Modellrechnung, keine Steuerberatung. Der AfA-Vergleich zeigt die Wirkung der Abschreibung. Unter „Miete & Finanzierung“ fließen zusätzlich deine Einnahmen, Kosten und das Darlehen in die Cashflow-Prognose ein. Die Annahmen gelten nur für dieses Szenario.</p></div>
