...
import { berlinYear, valuationDefaults } from "@/lib/valuations/somantic";
...
export default async function MarketPage({searchParams}:{searchParams:Promise<{property?:string}>}) {
  const viewer=await requireOrganization(); const supabase=await createClient(); const year=berlinYear(); const query=await searchParams;
  ...
  return <div className="space-y-6"><div><p className="text-sm text-muted-foreground">Somantic · Jahresbewertung {year}</p><h1 className="text-3xl font-semibold">Immobilienbewertung</h1><p className="mt-2 text-muted-foreground">Kaufpreis, mögliche Kaltmiete und Preisspannen auf Basis vergleichbarer Wohnimmobilien. Eine Bewertung pro Immobilie und Kalenderjahr.</p></div>
    {(properties.error || reports.error) ? <p role="alert">Die Bewertungsdaten konnten nicht geladen werden. Bitte erneut laden.</p> : <div className="grid gap-4 lg:grid-cols-2">{properties.data?.filter(p=>!query.property || p.id===query.property).map(p=>{
      const propertyReports=reports.data?.filter(r=>r.property_id===p.id) ?? []; const current=propertyReports.find(r=>r.valuation_year===year); const locked=current && current.status!=="failed";
      return <Card key={p.id}><CardHeader><CardTitle><Link href={`/app/immobilien/${p.id}`}>{p.name}</Link></CardTitle><p className="text-sm text-muted-foreground">{p.street} {p.house_number}, {p.postal_code} {p.city}</p></CardHeader><CardContent className="space-y-3">
        {locked ? <p>{["succeeded","insufficient"].includes(current.status) ? `Jahresbewertung ${year} liegt vor. Nächster Abruf ab 01.01.${year+1}.` : "Eine Anfrage wurde bereits gestartet. Bei unbestätigtem Status bitte den Support kontaktieren."}</p> : hasPermission(viewer.role,"portfolio.write") && <ValuationDialog propertyId={p.id} name={p.name} defaults={valuationDefaults(p,(units.data ?? []).filter(u=>u.property_id===p.id))} disabled={p.country_code!=="DE"}/>}
        ...
        {propertyReports.filter(r=>["succeeded","insufficient","uncertain"].includes(r.status)).map(r=><Link className="block text-sm underline" key={r.id} href={`/app/markt/${r.id}`}>Bewertungsbericht {r.valuation_year} öffnen</Link>)}
      </CardContent></Card>;
